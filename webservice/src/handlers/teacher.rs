...
pub async fn get_teacher_details(
    ...
    get_teacher_details_db(&app_state.db, teacher_id)
        .await
        .map(|_teacher| HttpResponse::Ok().json(teacher_id))
}
...
mod tests {
    ...
    #[actix_rt::test]
    async fn get_teacher_details_success_test() {
        dotenv().ok();
        let db_url = env::var("DATABASE_URL").expect("DATABASE_URL is not set ");
        let db_pool = PgPoolOptions::new().connect(&db_url).await.unwrap();
        let app_state: web::Data<AppState> = web::Data::new(AppState {
        ...
        let resp = get_teacher_details(app_state, web::Path::from(2)).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK)
    }
